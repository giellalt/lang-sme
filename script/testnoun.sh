...
grep ";" $GTHOME/gt/sme/src/noun-sme-lex.txt | grep -v "^\!" | egrep -v '(CmpN/Only|CmpN/Last|ShCmp|RCmpnd)' | sed 's/% /€/g' | sed 's/%:/¢/g' |  tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " | tr "¢" ":" | sort -u > nouns
...
grep ";" $GTHOME/gt/sme/src/noun-sme-lex.txt | grep -v "^\!" | grep -v ShCmp | egrep "( R | Rreal | Rnoun)" | cut -d ":" -f1 | sed 's/+Use\/Sub//' | sed 's/CmpN\/Pref+CmpN\/SgN$/Cmp#viessu+N+Sg+Nom/' |  sed 's/CmpN\/Pref+CmpN\/SgG$/Cmp#viessu+N+Sg+Nom/' | sed 's/CmpN\/PlG$/Cmp#viessu+N+Sg+Nom/' | $LOOKUP $GTHOME/gt/sme/bin/isme.fst | cut -f2 | grep "\-" | grep -v "\-.*\-" | cut -d "-" -f1 >> analnouns
grep ";" $GTHOME/gt/sme/src/noun-sme-lex.txt | grep -v "^\!" | grep -v ShCmp | egrep "( R | Rreal | Rnoun)" | cut -d ":" -f1 | sed 's/+Use\/Sub//' | sed 's/CmpN\/Pref+CmpN\/SgN$/Cmp#viessu+N+Sg+Nom/' |  sed 's/CmpN\/Pref+CmpN\/SgG$/Cmp#viessu+N+Sg+Nom/' | sed 's/CmpN\/PlG$/Cmp#viessu+N+Sg+Nom/' | $LOOKUP $GTHOME/gt/sme/bin/isme.fst | cut -f2 | grep "\-" | grep "\-.*\-" | cut -d "-" -f1,2 >> analnouns
