...
grep ";" adj-sme-lex.txt | grep -v "^\!" | egrep -v "(LEXICON|ATTR|FINJU|Der|Attr| At |BBO |MUS |BUOREMUSS|OVDDIT |STUORIBUS|BUStem | K | Rreal | R )" | tr ":+" " " | cut -d " " -f1 | tr -d "#" | sort -u > adjs
cat adjs | sed 's/$/+A+Sg+Nom/' | $LOOKUP $GTHOME/gt/sme/bin/isme.fst | cut -f2 | grep -v "A+" | grep -v "^$" | sort -u > analadjs 
grep ";" adj-sme-lex.txt | grep -v "^\!" | grep -v "+Gen" | egrep "(ATTR|Attr)" | tr ":+" " " | cut -d " " -f1 | tr -d "#" | sort -u > attradjs
cat attradjs | sed 's/$/+A+Attr/' | $LOOKUP $GTHOME/gt/sme/bin/isme.fst | cut -f2 | grep -v "A+" | grep -v "^$" | sort -u >> analadjs 
grep ";" adj-sme-lex.txt | grep -v "^\!" | grep "BUOREMUSS" | tr ":+" " " | cut -d " " -f1 | tr -d "#" | sort -u > superladjs
cat superladjs | sed 's/$/+A+Superl+Sg+Nom/' | $LOOKUP $GTHOME/gt/sme/bin/isme.fst | cut -f2 | grep -v "A+" | grep -v "^$" | sort -u >> analadjs 
grep ";" adj-sme-lex.txt | grep -v "^\!" | grep "(OVDDIT|BBO |BU/MUS|STUORIBUS|BUStem)" | tr ":+" " " | cut -d " " -f1 | tr -d "#" | sort -u > compladjs
