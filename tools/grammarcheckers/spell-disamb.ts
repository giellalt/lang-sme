...
export default function smeSpellDisamb(entry: StringEntry): Command {
    ...
    // The PROTECTing disambiguator, deliberately. Measured over 600 rows, the
    // de-PROTECTed variant cut candidate lists from 22 to 3.4 and took the gold
    // with them in 348 of them: CG disambiguation assumes a cohort's readings
    // are competing analyses of one word, whereas speller suggestions are
    // competing words, so SELECT deletes vocabulary rather than choosing.
    //
    // What is wanted instead is a disambiguated CONTEXT with the suggestions
    // left alone, so targeted rules downstream have unambiguous neighbours to
    // test against. PROTECT (<spelled>) gives exactly that.
    x = cg3.vislcg3("disamb", x, {
