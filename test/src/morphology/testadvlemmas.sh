...
grep ";" src/morphology/stems/adverbs.lexc | grep -v "^(\!|\@|<)" | egrep -v '(LexSub|-comp| LE|-sup |SUP| Px|^adv |^adv-plc)' |sed 's/% /€/g' | sed 's/%:/¢/g' |  tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " | tr "¢" ":" | sort -u > adverbs

# Hent ut lemmaer som krever Comp-tag. Lagres som compadverbs (se bz 1926)
grep ";" src/morphology/stems/adverbs.lexc | grep -v "^(\!|\@|<)" | egrep -v '(LexSub| Px|^adv |^adv-plc)' | egrep '(-comp| LE)' |sed 's/% /€/g' | sed 's/%:/¢/g' |  tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " | tr "¢" ":" > compadverbs

# Hent ut lemmaer som krever Superl-tag. Lagres som supadverbs (se bz 1926)
grep ";" src/morphology/stems/adverbs.lexc | grep -v "^(\!|\@|<)" | egrep -v '(LexSub| Px|^adv |^adv-plc)' | egrep '(-sup| SUP)' |sed 's/% /€/g' | sed 's/%:/¢/g' |  tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " | tr "¢" ":" > supadverbs
