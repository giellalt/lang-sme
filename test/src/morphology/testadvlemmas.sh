...
grep ";" src/morphology/stems/adverbs.lexc | egrep -v "^(\!|\@|<|\()" | egrep -v '(LexSub|-comp| LE|-sup |SUP| Px|^adv |^adv-plc)' |sed 's/% /€/g' | sed 's/%:/¢/g' |  tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " | tr "¢" ":" | sort -u > adverbs

# Hent ut lemmaer som krever Comp-tag. Lagres som compadverbs (se bz 1926)
grep ";" src/morphology/stems/adverbs.lexc | egrep -v "^(\!|\@|<|\()" | egrep -v '(LexSub| Px|^adv |^adv-plc)' | egrep '(-comp| LE)' |sed 's/% /€/g' | sed 's/%:/¢/g' |  tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " | tr "¢" ":" > compadverbs

# Hent ut lemmaer som krever Superl-tag. Lagres som supadverbs (se bz 1926)
grep ";" src/morphology/stems/adverbs.lexc | egrep -v "^(\!|\@|<|\()" | egrep -v '(LexSub| Px|^adv |^adv-plc)' | egrep '(-sup| SUP)' |sed 's/% /€/g' | sed 's/%:/¢/g' |  tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " | tr "¢" ":" > supadverbs

# Generer adverbformen av lemmaene i adverbs
sed 's/$/+Adv/' adverbs | $LOOKUP src/generator-gt-norm.xfst | cut -f2 | grep -v "Adv+" | grep -v "^$" > anadverbs 

# Generer adverbformen av lemmaene i compadverbs med +Adv+Comp (se bz 1926)
sed 's/$/+Adv+Comp/' compadverbs | $LOOKUP src/generator-gt-norm.xfst | cut -f2 | grep -v "Adv+" | grep -v "^$" >> anadverbs 

# Generer adverbformen av lemmaene i supadverbs med +Adv+Superl (se bz 1926)
sed 's/$/+Adv+Superl/' supadverbs | $LOOKUP src/generator-gt-norm.xfst | cut -f2 | grep -v "Adv+" | grep -v "^$" >> anadverbs 
...
comm -23 alladverbs anadverbs | grep -v '^$' | sed 's/$/+Adv/' | $LOOKUP src/generator-gt-norm.xfst > test/data/missingadverbLemmas.txt
