...
grep ";" ../../../src/morphology/generated_files/smi-sme-propernouns.lexc | grep -v "^\!" | egrep -v '(Attr|FirstTag|-atr|LexSub)' | sed 's/% /€/g' | sed 's/%:/¢/g' | sed 's/%!/!/g' | tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " | tr "¢" ":" | sort -u > props
...
cat props | sed 's/$/+N+Prop+Sg+Nom/' | $LOOKUP ../../../src/generator-gt-norm.xfst | cut -f2 | grep "N+" | cut -d "+" -f1 | sed 's/$/+N+Prop+Pl+Nom/' | $LOOKUP ../../../src/generator-gt-norm.xfst | cut -f2 | grep "N+" > ../../../test/data/missingProperLemmas.txt
comm -23 props analprops >> $GTHOME/gt/sme/dev/missingProperLemmas.txt
grep ";" ../../../src/morphology/generated_files/smi-sme-propernouns.lexc | grep -v "^\!" | egrep '(Attr|-atr|FirstTag)' | grep -v 'LexSub' | sed 's/% /€/g' |  tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " |  sort -u > attrprops
...
comm -23 attrprops analattrprops >> ../../../test/data/missingProperLemmas.txt
rm *props
